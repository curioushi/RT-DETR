...
class RTDETRPostProcessor(nn.Module):
    ...
    def forward(self, outputs, orig_target_sizes: torch.Tensor):
        logits, boxes, quads, masks = outputs['pred_logits'], outputs['pred_boxes'], outputs['pred_quads'], outputs['pred_masks']
        ...
        if self.deploy_mode:
            return labels, boxes, scores, quads, masks
        ...
        results = []
        for lab, box, sco, quad, mask in zip(labels, boxes, scores, quads, masks):
            result = dict(labels=lab, boxes=box, scores=sco, quads=quad, masks=mask)

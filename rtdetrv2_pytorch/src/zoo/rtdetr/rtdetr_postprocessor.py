...
class RTDETRPostProcessor(nn.Module):
    ...
    def forward(self, outputs, orig_target_sizes: torch.Tensor):
        logits, boxes, quads, weights = outputs['pred_logits'], outputs['pred_boxes'], outputs['pred_quads'], outputs['pred_weights']
        ...
        if self.deploy_mode:
            return labels, boxes, scores, quads, weights
        ...
        results = []
        for lab, box, sco, quad, weight in zip(labels, boxes, scores, quads, weights):
            result = dict(labels=lab, boxes=box, scores=sco, quads=quad, weights=weight)
